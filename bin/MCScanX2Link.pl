...
#Version 1.40    hewm@genomics.org.cn 

die  "Version 1.42\t2022-11-18;\nUsage: $0 <InA.gff><InB.gff><A2B.collinearity><OutPrefix>\n" unless (@ARGV ==4);
...
################ read the gene info from gff file #######################
while(<GffA>)
{
	chomp;
	next if ($_=~s/#/#/);
...
################ get the pairwise gene link info #######################
while(<SYS>)
{ 
	chomp ; 
	my @inf=split ;
...
######################Run the NGenomeSyn to plot the fig ###########################
...
if  ( !(-e $NGenomeSyn) )
{
	...
	if  ( !(-e $NGenomeSyn) )
	{
		$NGenomeSyn="$RealBin/NGenomeSyn";
	}
}

if  ( (-e $NGenomeSyn) )
{
	print "\t$NGenomeSyn  -InConf   $OutPrefix.conf   -OutPut    $OutPrefix.svg \n";
	system ("$NGenomeSyn  -InConf   $OutPrefix.conf   -OutPut    $OutPrefix.svg ");
}
else
{
...
######################End , ALL done ,best wishes , blessings ###########################
